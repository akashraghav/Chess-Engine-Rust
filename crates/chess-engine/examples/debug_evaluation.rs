...
use chess_core::{GameState, evaluation::Evaluator};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Evaluation with white queen advantage: {}", eval_white_advantage);
    
    let difference = eval_white_advantage - eval_kings_only;
    println!("   Difference: {} (should be ~900 for queen value)", difference);
    
    // Test 4: Check piece counting in unequal position
    let game_state_unequal = GameState::from_fen("rnb1kb1r/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1")?;
    let white_queens = game_state_unequal.position.pieces_of_type(chess_core::PieceType::Queen, chess_core::Color::White).count_bits();
    let black_queens = game_state_unequal.position.pieces_of_type(chess_core::PieceType::Queen, chess_core::Color::Black).count_bits();
    ...
    let current_white_queens = current_state.pieces_of_type(chess_core::PieceType::Queen, chess_core::Color::White).count_bits();
    let current_black_queens = current_state.pieces_of_type(chess_core::PieceType::Queen, chess_core::Color::Black).count_bits();
