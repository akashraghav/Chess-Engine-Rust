use chess_core::{Color, Evaluator, GameState, PieceType};
...
fn main() -> Result<()> {
    ...
    for piece_type in PieceType::ALL {
        ...
        println!(
            "  {:?}: White={}, Black={}, Value={}",
            piece_type, white_count, black_count, piece_value
        );
    ...
    println!(
        "  Expected material balance: {} - {} = {}",
        white_material, black_material, expected_material_balance
    );
    ...
    for piece_type in PieceType::ALL {
        let white_count = unbalanced_state
            .position
            .pieces_of_type(piece_type, Color::White)
            .count_bits() as i32;
        let black_count = unbalanced_state
            .position
            .pieces_of_type(piece_type, Color::Black)
            .count_bits() as i32;
        let piece_value = piece_type.value();
    ...
    println!(
        "  Expected material balance: {} - {} = {}",
        white_material_unbal, black_material_unbal, expected_material_balance_unbal
    );
