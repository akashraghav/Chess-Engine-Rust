...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(best_move) = engine.find_best_move()? {
        ...
        if move_str.contains("e5")
            || move_str.contains("d5")
            || move_str.contains("f6")
            || move_str.contains("c6")
            || move_str.contains("f5")
            || move_str.contains("c5")
        {
    ...
    if eval_with_queen > eval_without_queen + 500 {
        // Queen worth ~900, so should be significant
