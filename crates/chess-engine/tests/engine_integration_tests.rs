...
mod engine_integration_tests {
    ...
    fn test_multiple_fen_loads() -> Result<()> {
        ...
        for (fen, expected_range) in positions_and_expected {
            ...
            assert!(expected_range.contains(&eval), 
                   "FEN '{}' should evaluate in range {:?}, got {}", 
                   fen, expected_range, eval);
        }
    ...
    fn test_engine_from_fen_constructor() -> Result<()> {
        ...
        assert!(eval > 800, "Engine created from FEN should evaluate queen advantage correctly");
    ...
    fn test_tactical_awareness_integration() -> Result<()> {
        ...
        assert!(eval_missing_knight < eval_balanced, 
        ...
        assert!(diff > 250 && diff < 400, 
               "Missing knight should cost ~320 points, actual diff: {}", diff);
    ...
    fn test_evaluation_sign_convention() -> Result<()> {
        ...
        assert!(white_eval > 0, "White advantage should be positive: {}", white_eval);
        assert!(black_eval < 0, "Black advantage should be negative: {}", black_eval);
        
        // Should be roughly symmetric
        let symmetry_diff = (white_eval + black_eval).abs();
        assert!(symmetry_diff < 100, "Evaluations should be roughly symmetric, diff: {}", symmetry_diff);
    ...
    fn test_engine_error_handling() {
        ...
        assert!(result.is_err(), "FEN with wrong rank count should return error");
    }
    ...
    fn test_engine_state_isolation() -> Result<()> {
        ...
        assert!((eval1 - eval2).abs() > 1000, "Engines should have significantly different evaluations");
    ...
    fn test_evaluation_after_multiple_loads() -> Result<()> {
        ...
        assert!(evaluations[0].0.abs() <= 50, "Kings only should be near 0: {}", evaluations[0].0);
        assert!(evaluations[1].0 > 800, "White queen should be positive: {}", evaluations[1].0);
        assert!(evaluations[2].0.abs() <= 50, "Kings only again should be near 0: {}", evaluations[2].0);
        assert!(evaluations[3].0 < -800, "Black queen should be negative: {}", evaluations[3].0);
        
        // First and third should be the same (same position)
        let kings_diff = (evaluations[0].0 - evaluations[2].0).abs();
        assert!(kings_diff <= 5, "Same position should give same evaluation: diff {}", kings_diff);
