...
impl TranspositionEntry {
    ...
    pub fn new(zobrist: u64, evaluation: i32, depth: u8, node_type: NodeType, _age: u8, best_move: Move) -> Self {
...
impl TranspositionTable {
    ...
    pub fn store(&mut self, zobrist: u64, evaluation: i32, depth: u8, node_type: NodeType, best_move: Move) {
        let index = self.index_for_hash(zobrist);
        let current_age = self.age.load(Ordering::Relaxed);
        
        let new_entry = TranspositionEntry::new(zobrist, evaluation, depth, node_type, current_age, best_move);
        
        // Replacement scheme: always replace if empty, otherwise use depth-preferred replacement
        let existing = &self.entries[index];
        if existing.is_empty() || 
           self.should_replace(existing, &new_entry, current_age) {
    ...
    fn should_replace(&self, existing: &TranspositionEntry, new: &TranspositionEntry, current_age: u8) -> bool {
        // Replace if:
...
impl MovePool {
    ...
    pub fn get_moves(&self, count: usize) -> Option<&mut [Move]> {
