...
use crate::{Bitboard, Color, PieceType, Position, Square};
...
impl OptimizedEvaluator {
    ...
    pub fn evaluate_incremental(
        ...
        if let (Some(prev_hash), Some(move_item)) = (previous_hash, move_made) {
            if let Some(prev_eval) = self.evaluation_cache.get(&prev_hash) {
                if let Some(updated_eval) =
                    self.update_evaluation_incrementally(prev_eval, position, move_item)
                {
    ...
    fn compute_full_evaluation(&mut self, position: &Position) -> EvaluationCache {
        ...
        let pawn_structure_score = self
            .pawn_structure_cache
            .get(&pawn_hash)
            .copied()
            .unwrap_or_else(|| {
        ...
        let (white_mobility, black_mobility) = self
            .mobility_cache
            .get(&zobrist)
            .copied()
            .unwrap_or_else(|| {
    ...
    fn update_evaluation_incrementally(
        ...
        if let Some(piece) = position.piece_at(move_made.to) {
            ...
            let old_value =
                self.get_piece_square_value(piece_type, move_made.from, color, prev_eval.phase);
            // Add new position value
            let new_value =
                self.get_piece_square_value(piece_type, move_made.to, color, prev_eval.phase);
    ...
    fn interpolate_evaluation(&self, cached: &EvaluationCache, position: &Position) -> i32 {
        ...
        let side_to_move_bonus = if position.side_to_move() == Color::White {
            10
        } else {
            -10
        };
    ...
    fn get_or_compute_king_safety(&mut self, position: &Position, color: Color) -> i32 {
        ...
        self.king_safety_cache
            ...
            .unwrap_or_else(|| {
                ...
                self.king_safety_cache
                    .insert((king_pawn_hash, color), safety);
                safety
            })
    }
    ...
    fn evaluate_king_safety(&self, position: &Position, color: Color) -> i32 {
        ...
        for file_offset in -1..=1 {
            ...
            if (0..8).contains(&file) && !self.has_pawn_on_file(position, file as u8, color) {
                safety_score -= 20;
            }
        }
