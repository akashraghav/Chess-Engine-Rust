...
mod evaluation_regression_tests {
    use chess_core::{GameState, Evaluator, PieceType};
    ...
    fn test_starting_position_evaluation() {
        ...
        assert!(eval.abs() <= 50, "Starting position should be near 0, got {}", eval);
    }
    ...
    fn test_material_advantage_white_queen() {
        ...
        assert!(eval > 800, "White queen advantage should be > 800, got {}", eval);
        assert!(eval < 1200, "White queen advantage should be < 1200, got {}", eval);
    }
    ...
    fn test_material_advantage_black_queen() {
        ...
        assert!(eval < -800, "Black queen advantage should be < -800, got {}", eval);
        assert!(eval > -1200, "Black queen advantage should be > -1200, got {}", eval);
    }
    ...
    fn test_material_advantage_white_rook() {
        ...
        assert!(eval > 400, "White rook advantage should be > 400, got {}", eval);
        assert!(eval < 700, "White rook advantage should be < 700, got {}", eval);
    }
    ...
    fn test_material_advantage_symmetry() {
        ...
        assert!(diff <= 50, "Evaluations should be symmetric within 50 points: white={}, black={}, diff={}", 
                white_eval, black_eval, diff);
    }
    ...
    fn test_piece_value_ordering() {
        ...
        assert!(queen_eval > rook_eval, "Queen should be worth more than rook");
        assert!(rook_eval > bishop_eval, "Rook should be worth more than bishop");
        assert!(rook_eval > knight_eval, "Rook should be worth more than knight");
        assert!(bishop_eval > pawn_eval, "Bishop should be worth more than pawn");
        assert!(knight_eval > pawn_eval, "Knight should be worth more than pawn");
        ...
        assert!(bishop_knight_diff >= 10, "Bishop-knight difference should be at least base value diff (10)");
        assert!(bishop_knight_diff <= 150, "Bishop-knight positional difference should be reasonable, got: {}", bishop_knight_diff);
        
        // More importantly, verify the base piece values are correct
        assert_eq!(PieceType::Bishop.value(), 330, "Bishop base value should be 330");
        assert_eq!(PieceType::Knight.value(), 320, "Knight base value should be 320");
    }
    ...
    fn test_known_tactical_positions() {
        ...
        let evaluator = Evaluator::new();
        let advantage_eval = evaluator.evaluate(&GameState::from_fen(material_advantage_fen).unwrap());
        ...
        assert!(advantage_eval > balanced_eval, 
    ...
    fn test_positional_evaluation_makes_sense() {
        ...
        let bishop_center = evaluator.evaluate(&GameState::from_fen("4k3/8/8/8/3B4/8/8/4K3 w - - 0 1").unwrap());
        let bishop_corner = evaluator.evaluate(&GameState::from_fen("4k3/8/8/8/8/8/8/B3K3 w - - 0 1").unwrap());
        let empty_pos = evaluator.evaluate(&GameState::from_fen("4k3/8/8/8/8/8/8/4K3 w - - 0 1").unwrap());
        ...
        assert!(bishop_center_bonus > bishop_corner_bonus, 
        ...
        assert!(bishop_center_bonus > 300, "Bishop in center should have significant value");
        assert!(bishop_corner_bonus > 300, "Bishop in corner should still have material value");
    }
    ...
    fn test_endgame_positions() {
        ...
        assert!(kp_eval > kk_eval, "King+pawn should be better than just kings");
