...
mod performance_tests {
    ...
    fn benchmark_move_generation_performance() {
        ...
        for _ in 0..iterations {
            ...
            assert!(
                moves.is_empty() || !moves.is_empty(),
                "Move generation should complete without panic"
            );
        }
        ...
        assert!(
            moves_per_second > 10_000,
            "Move generation too slow: {} moves/sec",
            moves_per_second
        );
    }
    ...
    fn benchmark_evaluation_performance() {
        ...
        assert!(
            eval_per_second > 5_000,
            "Evaluation too slow: {} eval/sec",
            eval_per_second
        );
    }
    ...
    fn benchmark_search_performance() {
        ...
        for _ in 0..iterations {
            ...
            assert!(
                result.best_move.is_none() || result.best_move.is_some(),
        ...
        assert!(
            searches_per_second > 0.5,
            "Search too slow: {:.2} searches/sec",
            searches_per_second
        );
    }
    ...
    fn benchmark_parallel_performance() {
        ...
        for _ in 0..iterations {
            ...
            assert!(
                result.0.is_none() || result.0.is_some(),
        ...
        println!(
            "✅ Completed {} parallel searches (depth {}, 4 threads) in {:.2}ms",
            iterations,
            depth,
            duration.as_millis()
        );
        ...
        assert!(
            searches_per_second > 0.5,
            "Parallel search too slow: {:.2} searches/sec",
            searches_per_second
        );
    }
}
